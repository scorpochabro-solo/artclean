...
- **`/`** — основная (editorial): манифест-режиссура, услуги-аккордеон.
- **`/v2`** — вариант по «продающей» структуре (hero с выгодами → сегментация по проблеме →
  по типу пространства → услуги → форма → результат → видео-тизер → преимущества →
  о компании → мастера → форма+контакты). Секции — в `components/v2/`, данные —
  `content/v2.ts`. Закрыта от индексации (без дублей контента), в sitemap не включена.
  Шапка сама подставляет навигацию нужной версии (`usePathname`).
